...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    """
    _LOGGER.info("Registering default OAuth credentials for Heiman Home")
    try:
        ...
        _LOGGER.error("Failed to register default OAuth credentials: %s", err)
    return True
...
async def async_setup_entry(hass: HomeAssistant, entry: HeimanConfigEntry) -> bool:
    ...
    # Store API client in hass.data for services to access
