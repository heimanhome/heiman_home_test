...
class HeimanButtonEntity(CoordinatorEntity[HeimanDataUpdateCoordinator], ButtonEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        _LOGGER.debug(
            "Button %s received coordinator update, value=%s",
            self._property_identifier,
            prop.value,
        )
        
        # Fire an event to Home Assistant event bus for activity tracking
        self.hass.bus.fire(
            f"{DOMAIN}_button_updated",
            {
                "device_id": device.device_id,
                "device_name": device.device_name,
                "property": self._property_identifier,
                "value": prop.value,
    ...
    async def async_press(self) -> None:
        ...
        # Fire an event for button press activity
        self.hass.bus.fire(
