            ...
            for property_id, prop in device.properties.items():
                ...
                if hasattr(prop, "entity") and prop.entity == "select":
                    ...
                    if unique_id not in existing_entities:
                        _LOGGER.debug(
                            "Creating select entity for %s: %s (value=%s)",
                            device.device_name,
                            property_id,
                            prop.value,
                        )
...
class HeimanSelectEntity(CoordinatorEntity[HeimanDataUpdateCoordinator], SelectEntity):
    ...
    def _setup_property_options(self, prop: DeviceProperty) -> None:
        ...
        """
        _LOGGER.debug(
            "Setting up options for property: %s (entity=%s, data_type=%s, value=%s)",
        ...
        if self._property_identifier == "AlarmSoundOption":
        ...
        elif self._property_identifier == "TUI":
            # Temperature Unit: Celsius/Fahrenheit
            _LOGGER.info("Setting up TUI (Temperature Unit) select options")
            self._attr_options = ["°C", "°F"]
            ...
            _LOGGER.warning(
                "Using default select options for unknown property: %s",
                self._property_identifier,
            )
