...
class HeimanSensorEntity(CoordinatorEntity[HeimanDataUpdateCoordinator], SensorEntity):
    ...
    def _apply_sensor_config(
        ...
        if config and matched_key:
            ...
            # Don't set unit for temperature sensors - it will be handled dynamically
            # based on TUI setting in native_unit_of_measurement property
            if self._property_identifier != "CurrentTemperature":
    ...
    def native_unit_of_measurement(self) -> str | None:
        ...
        """
        if self._property_identifier == "CurrentTemperature":
            # Get TUI value from device properties to determine unit
            device = self.coordinator.get_device(self._device.device_id)
            if device:
                tui_prop = device.properties.get("TUI")
                if tui_prop and tui_prop.value is not None:
                    try:
                        tui_value = int(tui_prop.value)
                        # Return unit based on TUI value
                        return "°F" if tui_value == 1 else "°C"
    ...
    def native_value(self) -> str | int | float | None:
        ...
        # Special handling for temperature with TUI unit conversion
        if self._property_identifier == "CurrentTemperature":
            ...
            if value is not None and isinstance(value, (int, float)):
                try:
                    # convert_temperature expects:
                    # - tui_value=0: return as-is (Celsius)
                    # - tui_value=1: convert Celsius to Fahrenheit
                    converted_value = convert_temperature(value, tui_value)
                    _LOGGER.debug(
                        "Converted temperature %s°C with TUI %s to %s",
                        value,
                        tui_value,
                        converted_value,
                    )
                    ...
                    _LOGGER.warning(
                        "Failed to convert temperature %s with TUI %s: %s",
                        value,
                        tui_value,
                        err,
                    )
